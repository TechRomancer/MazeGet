...
public class Camera {
	 ...
	 */
	public Camera(GameContainer gc, int width, int height, int tileSize) {
		this.numTilesX = width / tileSize;
		this.numTilesY = height / tileSize;
	...
	public void centerOn(float x, float y) {
		...
		cameraX = x - (gc.getWidth() / 2) / Globals.gameScale;
		cameraY = y - (gc.getHeight() / 2) / Globals.gameScale;
	...
	public void centerOn(Entity e) {
		float eCenterX = e.x + (e.width / 2);
		float eCenterY = e.y + (e.height / 2);
		this.centerOn(eCenterX, eCenterY);
	}
